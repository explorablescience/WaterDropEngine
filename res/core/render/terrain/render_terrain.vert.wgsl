#include "core/render/pbr/pbr_functions.wgsl"
#include "core/render/terrain/terrain_common.wgsl"

struct ModelInput {
    @location(0) position:  vec3<f32>,
    @location(1) tex_coord: vec2<f32>,
    @location(2) normal:    vec3<f32>,
    @location(3) tangent:   vec4<f32>
};
struct VertexOutput {
    @builtin(position) clip_position: vec4<f32>,
    @location(0) tex_coord:       vec2<f32>,
    @location(1) normal:          vec3<f32>,
    @location(2) normal_world:    vec3<f32>,
    @location(3) tangent_world:   vec4<f32>,
    @location(4) bitangent_world: vec3<f32>,
    @location(5) view_z:          f32,
    @location(6) tile_pos:        vec2<f32>,
    @location(7) tile_layer:      u32
};

struct Camera {
    world_to_view: mat4x4<f32>,
    view_to_ndc:   mat4x4<f32>
}
@group(0) @binding(0) var<uniform> in_camera: Camera;

@group(1) @binding(8) var material_displacement:         texture_2d_array<f32>;
@group(1) @binding(9) var material_displacement_sampler: sampler;

@group(2) @binding(0) var<uniform> in_terrain_description: TerrainDescription;
struct TerrainTile {
    pos:        vec2<f32>,
    lod:        f32,
    tile_layer: u32
}
@group(2) @binding(1) var<storage, read> in_terrain_tiles: array<TerrainTile>;

@group(3) @binding(0) var in_heightmap:         texture_2d_array<f32>;
@group(3) @binding(1) var in_heightmap_sampler: sampler;
@group(3) @binding(2) var in_splatmap:          texture_2d_array<f32>;
@group(3) @binding(3) var in_splatmap_sampler:  sampler;

@vertex
fn main(@builtin(instance_index) instance: u32, model: ModelInput) -> VertexOutput {
    var out: VertexOutput;

    let tile  = in_terrain_tiles[instance];
    let layer = i32(tile.tile_layer);

    // World offset for this chunk.
    let tile_offset = tile.pos * in_terrain_description.tile_size.xz;
    let obj_to_world = mat4x4<f32>(
        vec4<f32>(1.0, 0.0, 0.0, 0.0),
        vec4<f32>(0.0, 1.0, 0.0, 0.0),
        vec4<f32>(0.0, 0.0, 1.0, 0.0),
        vec4<f32>(tile_offset.x, 0.0, tile_offset.y, 1.0)
    );
    var world_pos = obj_to_world * vec4<f32>(model.position, 1.0);

    // Height from the array layer for this chunk.
    let h = in_terrain_description.tile_size.y;
    world_pos.y = textureSampleLevel(
        in_heightmap, in_heightmap_sampler, model.tex_coord, layer, 0.0
    ).r * h - 0.02;

    // Per-material displacement blended by the splatmap.
    let splatmap = textureSampleLevel(
        in_splatmap, in_splatmap_sampler, model.tex_coord, layer, 0.0
    );
    let weights      = vec4<f32>(splatmap.r, splatmap.g, splatmap.b, splatmap.a);
    let layer_count  = terrain_layer_count(textureNumLayers(material_displacement));
    var displacement = 0.0;
    for (var mat_layer: u32 = 0u; mat_layer < layer_count; mat_layer = mat_layer + 1u) {
        let uv = get_terrain_uv(model.tex_coord, tile.pos, in_terrain_description, mat_layer);
        let w  = splat_weight(weights, mat_layer);
        let d  = textureSampleLevel(
            material_displacement, material_displacement_sampler, uv, mat_layer, 0.0
        ).r;
        displacement += w * (d - 0.5) * 2.0 * in_terrain_description.displacement_scales[mat_layer];
    }
    world_pos.y += displacement;

    // Clip space.
    let view_pos4 = in_camera.world_to_view * world_pos;
    let view_pos  = view_pos4.xyz / view_pos4.w;
    out.clip_position = in_camera.view_to_ndc * vec4<f32>(view_pos, 1.0);
    out.view_z = -view_pos.z;

    out.tex_coord   = model.tex_coord;
    out.tile_pos    = tile.pos;
    out.tile_layer  = tile.tile_layer;

    // Normal via finite differences.
    let epsilon  = 0.001;
    let height_l = textureSampleLevel(in_heightmap, in_heightmap_sampler, model.tex_coord + vec2<f32>(-epsilon, 0.0), layer, 0.0).r * h;
    let height_r = textureSampleLevel(in_heightmap, in_heightmap_sampler, model.tex_coord + vec2<f32>( epsilon, 0.0), layer, 0.0).r * h;
    let height_d = textureSampleLevel(in_heightmap, in_heightmap_sampler, model.tex_coord + vec2<f32>(0.0, -epsilon), layer, 0.0).r * h;
    let height_u = textureSampleLevel(in_heightmap, in_heightmap_sampler, model.tex_coord + vec2<f32>(0.0,  epsilon), layer, 0.0).r * h;
    let local_normal = normalize(vec3<f32>(height_l - height_r, 2.0 * epsilon, height_d - height_u));
    out.normal = local_normal;

    let nm = normal_matrix(obj_to_world);
    out.normal_world    = nm * local_normal;
    out.tangent_world   = vec4<f32>(nm * model.tangent.xyz, model.tangent.w);
    out.bitangent_world = cross(out.normal_world, out.tangent_world.xyz) * out.tangent_world.w;

    return out;
}
