...
impl DisplayTexturePass {
    pub fn extract(
        query: ExtractWorld<Query<(&DisplayTextureMaterial, &Mesh3d)>>,
...
//             let materials = world.get_resource::<RenderAssets<GpuRenderBinding<DisplayTextureMaterialAsset>>>().unwrap();
//             if let (
