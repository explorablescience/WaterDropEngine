...
fn edit_camera_params(ctx: Res<EguiContext>, mut camera_query: Query<(&mut CameraView, &mut ThirdPersonController), With<ActiveCamera>>) {
    if let Ok((mut view, mut controller)) = camera_query.single_mut() {
        ...
        .resizable(false)
        .default_pos(egui::pos2(100.0, 10.0))
        .show(&ctx.0, |ui| {
            ui.add(egui::Slider::new(&mut view.fov, 1.0..=179.0).text("FOV").suffix("°"));
            ui.add(egui::Slider::new(&mut view.znear, 0.01..=10.0).text("Near Plane").suffix(" units"));
            ui.add(egui::Slider::new(&mut view.zfar, 10.0..=10000.0).text("Far Plane").suffix(" units"));
            ui.separator();
