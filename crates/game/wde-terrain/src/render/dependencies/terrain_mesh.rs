...
use crate::manager::{TILE_SIZE, RENDER_TILE_SUBDIVISIONS};
...
impl TerrainRenderPassMesh {
    ...
    pub fn init(assets_server: Res<AssetServer>, mut render_pass: ResMut<TerrainRenderPassMesh>) {
        let mut mesh = PlaneMesh::from("terrain_tile", RENDER_TILE_SUBDIVISIONS, Vec3::Y);

        // Scale the plane to cover the entire terrain tile size
        for vertex in &mut mesh.vertices {
            vertex.position[0] *= TILE_SIZE[0];
            vertex.position[2] *= TILE_SIZE[2];
        }

        // Fix the UVs to cover the entire texture
        for vertex in &mut mesh.vertices {
            vertex.uv[0] = vertex.position[0] / TILE_SIZE[0] + 0.5; // Map from [-TILE_SIZE/2, TILE_SIZE/2] to [0, 1]
            vertex.uv[1] = vertex.position[2] / TILE_SIZE[2] + 0.5; // Map from [-TILE_SIZE/2, TILE_SIZE/2] to [0, 1]
        }
