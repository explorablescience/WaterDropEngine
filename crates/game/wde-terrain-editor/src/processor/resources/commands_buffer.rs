...
impl Plugin for ComputeCommandsBufferPlugin {
    fn build(&self, app: &mut App) {
        app.get_sub_app_mut(RenderApp).unwrap()
            ...
            .add_systems(Render, build_commands_bind_group.in_set(RenderSet::BindGroups))
...
fn build_commands_bind_group(render_instance: Res<RenderInstance>, mut commands_buffer: ResMut<CommandsBuffer>, buffers: Res<RenderAssets<GpuBuffer>>) {
...
fn update_commands_buffer(
    render_instance: Res<RenderInstance>, mut commands_buffer: ResMut<CommandsBuffer>, buffers: Res<RenderAssets<GpuBuffer>>,
    ...
    // Prepare the data
    let data: Vec<CommandDescription> = extracted_commands.commands.iter().map(|command| {
        CommandDescription {
