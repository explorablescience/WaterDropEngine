...
fn extract_terrain_textures(
    ...
    if terrain_materials_world.albedo_textures.is_empty() || terrain_materials_world.albedo_array.is_none() {
        return;
    }
...
fn build_material_arrays(
    ...
    // Check if bind group is already built or if all textures are loaded
    if material_arrays.bind_group.is_some() || (materials.albedo_textures.iter().any(|tex| tex.is_none()) || materials.albedo_array.is_none()) {
        return;
    }

    // Get the array texture
    let (albedo_array, normal_array, roughness_array, ao_array) = match (
        (materials.albedo_array.as_ref().and_then(|handle| textures.get(handle))),
        (materials.normal_array.as_ref().and_then(|handle| textures.get(handle))),
        (materials.roughness_array.as_ref().and_then(|handle| textures.get(handle))),
        (materials.ao_array.as_ref().and_then(|handle| textures.get(handle))),
    ) {
    ...
    for i in 0..materials.albedo_textures.len() {
        ...
        albedo_array.texture.copy_from_texture_layered(&render_instance, &albedo_tex.texture.texture, i, size);

        let normal_tex = match textures.get(materials.normal_textures[i].as_ref().unwrap()) {
            Some(tex) => tex,
            None => return,
        };
        normal_array.texture.copy_from_texture_layered(&render_instance, &normal_tex.texture.texture, i, size);

        let roughness_tex = match textures.get(materials.roughness_textures[i].as_ref().unwrap()) {
            Some(tex) => tex,
            None => return,
        };
        roughness_array.texture.copy_from_texture_layered(&render_instance, &roughness_tex.texture.texture, i, size);
