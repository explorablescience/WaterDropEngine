...
pub const CHUNK_GRID_SUBDIVISIONS: u32 = CHUNK_SIZE[0] as u32 / 2;
...
impl Grid {
    /// Clears all chunks and tiles from the grid, effectively resetting it to an empty state.
    ...
    pub fn get_entity_at(&self, world_pos: Vec3) -> Option<Entity> {
        let (chunk_pos, local_pos) = get_tile_position(world_pos)?;
        let chunk = self.chunks.get(&chunk_pos)?;
        chunk.get_local(local_pos)
    }

    /// Gets a reference to the entity of the tile at the specified chunk and local position.
    /// 
    /// # Arguments
    ...
    /// # Returns
    /// An option containing a reference to the Entity of the tile if it exists, or None if the tile does not exist or is out of bounds.
    pub fn get_entity_at_chunk_local(&self, chunk_pos: GridChunkPos, local_pos: GridLocalPos) -> Option<Entity> {
        let chunk = self.chunks.get(&chunk_pos)?;
        chunk.get_local(local_pos)
    }
    ...
    pub fn set_entity_at(&mut self, world_pos: Vec3, entity: Entity) {
        let (chunk_pos, local_pos) = match get_tile_position(world_pos) {
            Some(pos) => pos,
            None => // Create the necessary chunk if it doesn't exist
                get_tile_position(world_pos).unwrap_or_else(|| {
                    ...
                    get_tile_position(world_pos).expect("Chunk should have been created")
                })
        };
