...
fn init_placement(asset_server: Res<AssetServer>, mut placement_config_struct: ResMut<PlacementConfig>) {
    // Load placement configuration file
    let placement_config = match serialize::parse_file(PLACEMENT_CONFIG_PATH) {
        Ok(config) => config,
        Err(err) => {
            error!("Failed to load placement configuration file at '{}': {}", PLACEMENT_CONFIG_PATH, err);
            return;
        }
    };
    ...
    if placement_config["version"].as_u64().unwrap_or(0) != PLACEMENT_CONFIG_VERSION as u64 {
        error!("Unsupported placement configuration version: {}. Expected version: {}", placement_config["version"], PLACEMENT_CONFIG_VERSION);
        return;
    }
    ...
    for entity in config_entities.unwrap_or(&vec![]) {
        ...
        let (label, model_path, extent) = match (
            ...
            (Some(label), Some(model_path), Some(extent_x), Some(extent_y)) => (label.to_string(), model_path.to_string(), UVec2::new(extent_x as u32, extent_y as u32)),
            _ => {
                error!("Invalid entity configuration in placement config: {:?}", entity);
                continue;
            }
        };
        ...
        entities.push(PlacementConfigEntry { label: label.clone(), asset: gltf_model, extent });
        labels.push(label);
    }
