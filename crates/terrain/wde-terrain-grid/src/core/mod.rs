...
use crate::core::{entries::PlacementPlugin, grid::{Grid, GridEntityEvent}};
...
fn handle_grid_entity_events(
    mut grid: ResMut<Grid>,
    mut events: MessageReader<GridEntityEvent>
) {
    for event in events.read() {
        match event {
            GridEntityEvent::Placed { entity, grid_entity } => {
