...
use wgpu::{
    BackendOptions, Device, Features, Limits as WLimits, MemoryBudgetThresholds,
    PresentMode as WPresentMode, Surface, SurfaceConfiguration, SurfaceTexture
};
...
pub async fn create_instance(
    ...
    if adapter_info.device_type == wgpu::DeviceType::Cpu {
        warn!(
            "The selected adapter is using a driver that only supports software rendering, this will be very slow."
        );
    }

    // Set required features
    let required_features =
        wgpu::Features::TEXTURE_ADAPTER_SPECIFIC_FORMAT_FEATURES | Features::PUSH_CONSTANTS;
    ...
    device.on_uncaptured_error(std::sync::Arc::new(|error| match error {
        wgpu::Error::OutOfMemory { source } => {
            error!("Out of memory error from wgpu: {:#?}", source)
        }
        wgpu::Error::Validation {
            description,
            source
        } => warn!(
            "Uncaptured validation error from wgpu.\n{}\n{:#?}",
            description, source
        ),
        wgpu::Error::Internal {
            source,
            description
        } => error!("Internal error from wgpu: {}\n{:#?}", description, source)
    }));
    ...
    device.set_device_lost_callback(|reason, message| match reason {
        ...
        wgpu::DeviceLostReason::Unknown => {
            error!("The GPU device was lost for an unknown reason: {}", message)
        }
    });
...
pub fn get_current_texture(
    ...
    match render_texture {
        Ok(surface_texture) => {
            // Create render view
            let render_view = surface_texture
                .texture
        ...
        Err(wgpu::SurfaceError::Lost | wgpu::SurfaceError::Outdated) => RenderEvent::Resize,
