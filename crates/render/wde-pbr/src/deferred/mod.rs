...
//!
//! ## GBuffer pass (see [`RenderPassDeferredGBuffer`](crate::passes::RenderPassDeferredGBuffer))
...
//! - When a transform or a model is added or changed in the main world, the corresponding transform is marked as dirty, and the [SsboTransform](crate::SsboTransform) buffer is updated with the new transform data at the next frame.
...
//! - Finally, the render pass iterates over the batches and renders them, writing to the G-buffer [`DeferredTextures`](crate::DeferredTextures) and the depth texture.
//! - It then resolves the G-buffer textures to `DeferredTexturesResolved` for use in the lighting pass, which doesn't use MSAA.
//!
//! ## Lighting pass (see [`RenderPassDeferredLighting`](crate::passes::RenderPassDeferredLighting))
...
//! - Then, for each frame, it reads the G-buffer textures and the light data, and computes the final lit colors, which it writes to a [`RenderTexture`](crate::prelude::RenderTexture) in MSAA format.
//!
//! ## Limitations
//! The number of supported lights and tracked PBR transform entries is limited by:
//!   - [`MAX_LIGHTS`](crate::MAX_LIGHTS) limits the GPU light buffer size.
//!   - [`SSBO_TRANSFORM_MAX_ENTITY`](crate::SSBO_TRANSFORM_MAX_ENTITY) limits tracked PBR transform entries.
//!
//! ## Custom rendering
//! To find out more about how to create custom subpasses, materials and render pipelines, see the documentation of the [`wde_renderer`](wde_renderer) crate.
//!
//! # Usage
//! ```rust
