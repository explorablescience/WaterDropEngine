...
pub(crate) fn set_instances_to_transform(
    ...
    let (staging_buffer, gpu_buffer) = match (
        buffers.get(&instance_to_transform.get_buffer(PbrSsboInstanceToTransform::INSTANCE_TO_TRANSFORM_STAGING_IDX).unwrap()),
        buffers.get(&instance_to_transform.get_buffer(PbrSsboInstanceToTransform::INSTANCE_TO_TRANSFORM_IDX).unwrap()),
    ) {
    ...
    for batch_key in batches.dirty_batches.clone().iter() {
        ...
        for transform_id in &batch.transform_ssbo_ids {
            ...
            staging_content[(offset as usize * 4)..((offset as usize + 1) * 4)].copy_from_slice(&bytes);
            offset += 1;
        }
    }
    let render_instance = render_instance.0.read().unwrap();
    staging_buffer.buffer.write(&render_instance, &staging_content, 0);

    // Copy the staging buffer to the GPU buffer
    gpu_buffer.buffer.copy_from_buffer(&render_instance, &staging_buffer.buffer);
