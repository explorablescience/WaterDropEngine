...
impl WdeManifest {
    /// Calls `f` with a global shared instance of the [`WdeManifest`] struct.
    pub fn shared<R>(f: impl FnOnce(&WdeManifest) -> R) -> R {
        static MANIFESTS: RwLock<BTreeMap<PathBuf, WdeManifest>> = RwLock::new(BTreeMap::new());
