...
use crate::{assets::{RenderAsset, RenderAssetsPlugin}, core::RenderApp};
...
impl<P: RenderAsset + TypePath + Default> Plugin for RenderPipelinePluginRegister<P> {
    fn build(&self, app: &mut App) {
        ...
        app
            .init_asset::<RenderPipelineAsset<P>>()
    ...
    fn finish(&self, app: &mut App) {
        ...
        let pipeline = app.world_mut()
            .get_resource::<AssetServer>().unwrap().add(RenderPipelineAsset::<P>::default());

        // Spawn an entity holding the render pipeline asset handle, avoiding it to be dropped and unloaded
        app.get_sub_app_mut(RenderApp).unwrap()
            .world_mut().spawn(RenderPipelineAssetHolder(pipeline));
    }
}
