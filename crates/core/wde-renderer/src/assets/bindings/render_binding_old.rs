...
pub type SBindingOld<M> = SRes<RenderAssets<GpuRenderBindingOld<M>>>;
/// Alias for a `SResMut<RenderAssets<GpuRenderBinding<M>>>`.
pub type SBindingMutOld<M> = SResMut<RenderAssets<GpuRenderBindingOld<M>>>;

/// Alias for a `Res<RenderAssets<GpuRenderBinding<M>>>`.
pub type BindingOld<'w, M> = Res<'w, RenderAssets<GpuRenderBindingOld<M>>>;
/// Alias for a `ResMut<RenderAssets<GpuRenderBinding<M>>>`.
pub type BindingMutOld<'w, M> = ResMut<'w, RenderAssets<GpuRenderBindingOld<M>>>;

/// Utility resource to store a handle to a render binding asset [RenderBinding].
#[allow(unused)]
#[derive(Resource, Clone)]
pub struct RenderBindingHolderOld<M: RenderBindingOld>(pub Handle<M>);

/// Plugin to register a custom [`RenderBinding`] type as an asset, and prepare the corresponding [`GpuRenderBinding`] render asset.
pub struct RenderBindingPluginRegisterOld<M: RenderBindingOld> {
    _phantom: std::marker::PhantomData<M>,
    default_init: bool
}
impl<M: RenderBindingOld> RenderBindingPluginRegisterOld<M> {
...
impl<M: RenderBindingOld> Default for RenderBindingPluginRegisterOld<M> {
    ...
    fn default() -> Self {
        RenderBindingPluginRegisterOld {
            _phantom: std::marker::PhantomData,
            default_init: true
        }
    }
}
impl<M: RenderBindingOld + Default> Plugin for RenderBindingPluginRegisterOld<M> {
    fn build(&self, app: &mut App) {
        app.init_asset::<M>()
            .add_plugins(RenderAssetsPlugin::<GpuRenderBindingOld<M>>::default());
    }
    fn finish(&self, app: &mut App) {
        if self.default_init {
            ...
            app.get_sub_app_mut(RenderApp)
                .unwrap()
                .insert_resource(RenderBindingHolderOld(binding));
        }
    }
}
...
pub trait RenderBindingOld: Asset + Clone + Sized {
...
pub struct GpuRenderBindingOld<M: RenderBindingOld> {
    _phantom: std::marker::PhantomData<M>,
    builder: RenderBindingBuilderOld,
...
impl<M: RenderBindingOld> GpuRenderBindingOld<M> {
...
impl<M: RenderBindingOld> RenderAsset for GpuRenderBindingOld<M> {
    type SourceAsset = M;
    type Params = (
        SRes<RenderInstance>,
    ...
    fn prepare(
        ...
        let mut bindings_builder = if let Some(builder) = renderbinding_cache.remove(&binding_label)
        {
            ...
            let mut builder = RenderBindingBuilderOld::default();
