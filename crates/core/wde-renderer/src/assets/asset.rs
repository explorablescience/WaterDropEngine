...
use bevy::{app::{App, Plugin}, ecs::{system::{StaticSystemParam, SystemParam, SystemParamItem, SystemState}, world}, platform::collections::{HashMap, HashSet}, prelude::*};
use thiserror::Error;
...
pub trait RenderAsset: Send + Sync + 'static + Sized {
    ...
    fn prepare(asset: Self::SourceAsset, params: &mut SystemParamItem<Self::Params>) -> Result<Self, PrepareAssetError<Self::SourceAsset>>;
...
impl<A: RenderAsset> RenderAssets<A> {
    pub fn get(&self, id: impl Into<AssetId<A::SourceAsset>>) -> Option<&A> { self.0.get(&id.into()) }
    pub fn get_mut(&mut self, id: impl Into<AssetId<A::SourceAsset>>) -> Option<&mut A> { self.0.get_mut(&id.into()) }
    pub fn insert(&mut self, id: impl Into<AssetId<A::SourceAsset>>, value: A) -> Option<A> { self.0.insert(id.into(), value) }
    pub fn remove(&mut self, id: impl Into<AssetId<A::SourceAsset>>) -> Option<A> { self.0.remove(&id.into()) }
    pub fn iter(&self) -> impl Iterator<Item = (&AssetId<A::SourceAsset>, &A)> { self.0.iter() }
    pub fn iter_mut(&mut self) -> impl Iterator<Item = (&AssetId<A::SourceAsset>, &mut A)> { self.0.iter_mut() }
}
...
struct CachedExtractAssetsState<A: RenderAsset> {
    state: SystemState<(MessageReader<'static, 'static, AssetEvent<A::SourceAsset>>, ResMut<'static, Assets<A::SourceAsset>>)>
}
impl<A: RenderAsset> FromWorld for CachedExtractAssetsState<A> {
    fn from_world(world: &mut world::World) -> Self { Self { state: SystemState::new(world) } }
}
...
impl<A: RenderAsset> Default for PrepareNextFrameAssets<A> {
    fn default() -> Self { Self { assets: Default::default() } }
}
...
impl<A: RenderAsset> Default for ExtractedAssets<A> {
    fn default() -> Self { Self { extracted: Default::default(), removed: Default::default(), added: Default::default() } }
}
