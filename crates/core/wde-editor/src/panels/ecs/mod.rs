...
impl Plugin for UIEcsPanelPlugin {
	fn build(&self, app: &mut App) {
		app.init_resource::<SelectedEntity>()
			...
			.add_systems(Startup, (init_ui, init_asset_catalog))
			.add_systems(
				Update,
				(
					draw_entities_panel,
					draw_selected_entity_components_panel,
					draw_assets_panel,
				),
			);
	}
...
fn init_asset_catalog(mut asset_catalog: ResMut<AssetCatalog>) {
...
fn draw_entities_panel(
	...
	let entity_set = rows.iter().map(|(entity, _, _)| *entity).collect::<Vec<_>>();
...
fn draw_selected_entity_components_panel(world: &mut World) {
