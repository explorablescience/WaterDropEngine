...
use crate::{prelude::*, ui_textures::{UITextureHandle, UITextures}};
use bevy::prelude::*;
...
impl Plugin for RenderGraphPanelPlugin {
    ...
    fn finish(&self, app: &mut App) {
        ...
        let render_graph = app.get_sub_app_mut(RenderApp).unwrap()
            .world().resource::<RenderGraph>();
        let mut names = vec![];
        ...
        app.world_mut().insert_resource(RenderGraphUIState {
            names, ghost_swapchain_texture_handle: None
         });
    }
}
...
fn init_render(
    ...
    let ghost_swapchain_texture_handle = ui_textures
...
fn draw_ui(
    ...
    *render_messages = ScreenshotRequest { request: false };
    egui::Window::new("Render Graph")
        ...
        .open(ui_menu.clicked_mut("Engine/Render Graph").unwrap_or(&mut false))
        .show(&ctx.0, |ui| {
...
fn extract(
    ...
    *render_messages = ScreenshotRequest { request: main_messages.request };
    render_graph_ui_state.ghost_swapchain_texture_handle = main_render_graph_ui_state.ghost_swapchain_texture_handle.clone();
}
...
fn render_custom_passes(world: &mut World) {
    ...
    if world.resource::<RenderGraphUIState>().ghost_swapchain_texture_handle.is_none() {
    ...
    world.resource_scope(|world, graph: Mut<RenderGraph>| {
        ...
        for id in sorted_passes {
            ...
            let _span = debug_span!("ui_screenshot_render_pass_render", pass_id = id, pass_name = pass.name()).entered();
            pass.render(world);
            drop(_span);

            let _span = debug_span!("ui_screenshot_render_pass_blit", pass_id = id, pass_name = pass.name()).entered();
            // Get swapchain frame and its ghost texture
            let swapchain_frame = world.resource::<SwapchainFrame>().data.as_ref().unwrap();
            let ghost_texture_handle = world.resource::<RenderGraphUIState>().ghost_swapchain_texture_handle.clone();
            let ghost_texture = world.resource::<RenderAssets<GpuTexture>>().get(ghost_texture_handle.unwrap()).unwrap();

            // Blit the swapchain frame
            let render_instance = world.resource::<RenderInstance>().0.read().unwrap();
            let size = (render_instance.surface_config.as_ref().unwrap().width, render_instance.surface_config.as_ref().unwrap().height);
            ghost_texture.texture.copy_from_surface_texture(&render_instance, &swapchain_frame.texture, size);
            drop(_span);
        }
    });
}
